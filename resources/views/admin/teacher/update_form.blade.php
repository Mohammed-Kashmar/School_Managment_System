...
<section id="interface">
  @include('admin.partials.search')
    ...
    <form name="register" class="form-question shadow-lg rounded mb-3" method="POST" action="{{route('teacher.update',['id'=>$teacher->id])}}"  enctype="multipart/form-data">
      @csrf
      <h3 class="i-name text-center mb-4 text-white pb-4 shadow-lg" style="background:#141E30;">Update Teacher</h3>
      ...
      <div class="row mb-4">
          <div class="col">
            ...
            <label class="label-question form-label" for="form6Example1"><i class="fa fa-solid fa-user me-2"></i> First name</label>
              <input type="text" id="form6Example1" class="form-control shadow" name="first_name" value="{{ $teacher->first_name }}"/>
              @error('first_name')
          ...
          <div class="col">
            <div class="form-outline">
             <label class="label-question form-label" for="form6Example2"><i class="fa fa-solid fa-user me-2"></i> Last name</label>
              <input type="text" id="form6Example2" class="form-control shadow" name="last_name" value="{{ $teacher->last_name }}"/>
              @error('last_name')
              <div class="error">{{ $message }}</div>
             @enderror
            </div>
        ...
        <div class="row mb-4">
          <div class="col form-outline mb-4">
            <label class="label-question form-label" for="form6Example5"><i class="fa fa-solid fa-envelope me-2"></i>Email</label>
             <input type="email" id="form6Example5" class="form-control shadow" name="email" value="{{ $teacher->email }}"/>
             @error('email')
                <div class="error">{{ $message }}</div>
               @enderror
            </div>
        ...
        <div class="row mb-4">
         ...
         <div class="col form-outline mb-4">
          ...
          <input type="tel" id="form6Example6" class="form-control shadow" name="phone" value="{{ $teacher->phone }}"/>
          @error('phone')
                <div class="error">{{ $message }}</div>
               @enderror
        </div>
        ...
        <div class="form-outline mb-4">
          ...
          <input type="text" id="form6Example4" class="form-control shadow" name="address" value="{{ $teacher->address }}"/>
          @error('address')
                <div class="error">{{ $message }}</div>
               @enderror
        </div>
        ...
        <div class="col form-outline mb-4">
          ...
          <input type="tel" id="form6Example6" class="form-control shadow" name="experience" value="{{ $teacher->experience }}"/>
          @error('experience')
                <div class="error">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="form-outline mb-4">
         ...
         <input type="file" id="form6Example4" class="form-control shadow" name="image" value="{{ $teacher->image }}"/>
         @error('image')
                <div class="error">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="row mb-4 d-flex align-items-center ">
          ...
          <div class="col form-outline ">
           ...
           <input type="date" id="form6Example6" class="form-control shadow" name="birthdate" value="{{ $teacher->birthdate }}"/>
           @error('birthdate')
                <div class="error">{{ $message }}</div>
               @enderror
         </div>
         <div class="col form-outline mb-4 mt-5">
          <div class="form-check form-check-inline " style="display: inline-flex;">
            ...
            <input class="form-check-input" type="radio" name="gender" id="inlineRadio1" value="male" {{ $teacher->gender=="male"?'checked':'' }}>
          </div>
          <div class="form-check form-check-inline " style="display: inline-flex;">
            <label style="margin-right: 30px;" class="label-question form-check-label " for="inlineRadio2">female</label>
            <input class="form-check-input " type="radio" name="gender" id="inlineRadio2" value="female" {{ $teacher->gender=="female"?'checked':'' }}>
        ...
        <div class="tab-pane fade" id="nav-course" role="tabpanel" aria-labelledby="nav-course-tab" tabindex="0">
          ...
          @php $res = array_column($teacher->course->toArray(),'id'); @endphp
          @foreach ($courses as $course)
          <option value="{{ $course->id }}" {{ in_array($course->id,$res)?'selected':'' }}>{{ $course->name}}</option>            
          @endforeach
          @error('course_id')
          <div class="error">{{ $message }}</div>
         @enderror
