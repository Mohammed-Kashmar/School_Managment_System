...
<section id="interface">
  @include('admin.partials.search')
    ...
    <form name="register" class="form-question shadow-lg rounded mb-5" method="post" action="{{route('student.update',['id'=>$student->id])}}" enctype="multipart/form-data">
      @csrf

      
      <h3 class="i-name text-center mb-4  text-white pb-4 shadow-lg" style="background:#141E30;">Update Student</h3>
      ...
      <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-student" role="tabpanel" aria-labelledby="nav-student-tab" tabindex="0">
          <div class="col form-outline mb-4">
            ...
            <input type="number" id="form6Example6" class="form-control shadow" name="registration_number" value="{{ $student->registration_number }}"/>
          ...
          <div class="row mb-4">
            <div class="col">
              ...
              <label class="label-question form-label" for="form6Example1"><i class=" fa-solid fa-user me-2"></i> First name</label>
                <input type="text" id="form6Example1" class="form-control shadow" name="first_name" value="{{ $student->first_name }}"/>
                @error('first_name')
                  <div class="error">{{ $message }}</div>
                 @enderror
              </div>
            ...
            <div class="col">
              <div class="form-outline">
               <label class="label-question form-label" for="form6Example2"><i class="fa fa-solid fa-user me-2"></i> Last name</label>
                <input type="text" id="form6Example2" class="form-control shadow" name="last_name" value="{{ $student->last_name }}"/>
                @error('last_name')
                  <div class="error">{{ $message }}</div>
                 @enderror
              </div>
          ...
          <div class="row mb-4">
            <div class="col form-outline mb-4">
              <label class="label-question form-label" for="form6Example5"><i class="fa fa-solid fa-envelope me-2" ></i>Email</label>
               <input type="email" id="form6Example5" class="form-control shadow" name="email" value="{{ $student->email }}"/>
               @error('email')
                  <div class="error">{{ $message }}</div>
                 @enderror
             </div>
          ...
          <div class="row mb-4">
           ...
           <div class="col form-outline mb-4">
            ...
            <input type="number" id="form6Example6" class="form-control shadow" name="phone" value="{{ $student->phone }}"/>
            @error('phone')
                  <div class="error">{{ $message }}</div>
                 @enderror
          </div>
          <div class="col form-outline mb-4">
            ...
            <input type="number" id="form6Example6" class="form-control shadow" name="home_phone" value="{{ $student->home_phone }}"/>
            @error('home_phone')
                  <div class="error">{{ $message }}</div>
                 @enderror
          </div>
          ...
          <div class="form-outline mb-4">
            ...
            <input type="text" id="form6Example4" class="form-control shadow" name="address" value="{{ $student->address }}"/>
            @error('address')
                  <div class="error">{{ $message }}</div>
                 @enderror
          </div>
          ...
          <div class="form-outline mb-4">
           ...
           <input type="file" id="form6Example4" class="form-control shadow" name="image" value="{{ $student->image }}"/>
           @error('image')
                  <div class="error">{{ $message }}</div>
                 @enderror
          </div>
          ...
          <div class="row mb-4 d-flex align-items-center ">
            ...
            <div class="col form-outline ">
             ...
             <input type="date" id="form6Example6" class="form-control shadow" name="birthdate" value="{{ $student->birthdate }}"/>
             @error('birthdate')
                  <div class="error">{{ $message }}</div>
                 @enderror
           </div>
          ...
          <div class="row mb-4">
            <div class="col">
              ...
              <label class="label-question form-label" for="form6Example1"><i class="fa fa-solid fa-user me-2"></i>Parent's Full Name</label>
                <input type="text" id="form6Example1" class="form-control shadow" name="parent_full_name" value="{{ $student->getParent[0]->full_name }}"/>
            ...
            <div class="col">
              <div class="col form-outline mb-4">
                ...
                <input type="number" id="form6Example6" class="form-control shadow" name="parent_phone" value="{{ $student->getParent[0]->phone }}"/>
                @error('parent_phone')
                <div class="error">{{ $message }}</div>
               @enderror
              </div>
            ...
            <div class="col">
              <div class="col form-outline mb-4">
                <label class="label-question form-label" for="form6Example5"><i class="fa fa-solid fa-envelope me-2"></i>Parent Email</label>
                 <input type="email" id="form6Example5" class="form-control shadow" name="parent_email" value="{{ $student->getParent[0]->email }}"/>
                 @error('parent_email')
                 <div class="error">{{ $message }}</div>
                @enderror
                </div>
        ...
        <div class="tab-pane fade" id="nav-classroom" role="tabpanel" aria-labelledby="nav-classroom-tab" tabindex="0">
          <select class="form-select" aria-label="Default select example" name="classroom_id">
            <option selected>Choose Classroom</option>
            @foreach ($classrooms as $classroom)
            <option value="{{ $classroom->id }}" {{ $classroom->id==$student->classroom->id?'selected':'' }}>{{ $classroom->classroom_name}}</option>            
            @endforeach
